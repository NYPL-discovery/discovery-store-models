...
function _separateItemAndHoldingRecords (resource) {
  if (resource.item_statements && resource.item_statements.length) {
    // holding statements are prefixed 'h'
    resource.holding_statements = resource.item_statements.filter((s) => /^h\d/.test(s.s))
    // item statements are prefixed with optional partner code (e.g. 'c', 'p'), then 'i'
    resource.item_statements = resource.item_statements.filter((s) => /^[a-z]?i/.test(s.s))
  }
...
module.exports = {
  ...
  // For testing:
  _internal: {
    _separateItemAndHoldingRecords
  }
}
